...
function App() {
  ...
  const handleContinue = (e) => {
    ...
    if (step === 1) {
      if (username.trim() !== '') {
        setStep(2); // Avanza a la etapa de contraseña
      } else {
        alert('Por favor ingresa un usuario');
      }
    } else if (step === 2) {
      if (password.trim() !== '') {
        // Envía el formulario aquí
        alert(`Usuario: ${username}, Contraseña: ${password} Gracias por tus datos`);
        // Puedes enviar los datos a tu backend usando fetch o axios aquí
      } else {
